## Context
The [BLoC pattern](https://youtu.be/PLHln7wHgPE) is one of the most powerful and appreciated state management techniques for Flutter. Unfortunately, this is still far from "perfection". For instance, in a traditional BLoC architecture, modifying the state of a screen A starting from a UI event happening in a screen B is not a trivial task. This is why many developers are already trying to get to the next level by proposing "leaner" and more flexible approaches. One example is the [upstate](https://levelup.gitconnected.com/introducing-upstate-simplified-state-management-for-flutter-c987cac25b04) library, which introduces the possibility to "[update your state anywhere and your app updates everywhere](https://levelup.gitconnected.com/introducing-upstate-simplified-state-management-for-flutter-c987cac25b04)" while sacrificing one of the most interesting features of the BLoC pattern: the formal separation between UI and Business Logic.
...
- Any other custom state is rendered as a particular realization of the `ScreenNameInitialized` state, which is always the last state yielded by the `mapEventToState` method inside the `ScreenNameBloc`; 
...
- The `state` attribute of each `screenName` instance, which holds all the current custom states of the screens, can be modified from anywhere in the app by calling the method `yieldState(appHandler.screenName,ScreenNameState)`, where `ScreenNameState` is the new custom state being assigned to `ScreenName`;
