## Context
The BLoC pattern is one of the most powerful and appreciated state management techniques for Flutter. Nevertheless, many developers are already trying to get to the next level by searching for a "leaner" and more flexible approach. For instance, in a traditional BLoC architecture, modifying the state of a screen A starting from a UI event happening in a screen B is not a trivial issue. The [upstate](https://levelup.gitconnected.com/introducing-upstate-simplified-state-management-for-flutter-c987cac25b04) tries to solve this issue by introducing the possibility to "update your state anywhere and your app updates everywhere". On the other hand, it sacrifices one of the main features of the BLoC pattern: the formal separation between UI and Business Logic.
