## Context
The BLoC pattern is one of the most powerful and appreciated state management techniques for Flutter. Unfortunately, this is still far from "perfection". For instance, in a traditional BLoC architecture, modifying the state of a screen A starting from a UI event happening in a screen B is not a trivial task. This is why many developers are already trying to get to the next level by proposing "leaner" and more flexible approaches. The [upstate](https://levelup.gitconnected.com/introducing-upstate-simplified-state-management-for-flutter-c987cac25b04) library is one of these, introducing the possibility to "[update your state anywhere and your app updates everywhere](https://levelup.gitconnected.com/introducing-upstate-simplified-state-management-for-flutter-c987cac25b04)", while sacrificing one of the main features of the BLoC pattern: the formal separation between UI and Business Logic.
